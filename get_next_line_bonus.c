...
{
	char		*res;
	static char	prev_buff[1000][BUFFER_SIZE];

	if (fd == -1 || read(fd, 0, 0))
		return (NULL);
	res = "";
	if (*prev_buff[fd])
		if (check_prev_buff(&res, prev_buff[fd]))
			return (res);
	gnl(fd, prev_buff[fd], &res);
	if (*res)
		return (res);
	return (NULL);
}
...
//	int file = open("./cmake-build-debug/Testing/big_line_with_nl", O_RDONLY);
	int file = open("./cmake-build-debug/Testing/multiple_line_no_nl", O_RDONLY);
